...
def generate_initial_code():
    ...
    # Load the full DataFrame to identify categorical columns
    df_full = pd.read_csv('xdata.csv')
    categorical_columns = get_categorical_columns(df_full)
    
    # Capture the filename dynamically
    filename = 'xdata.csv'
    
    reasoning_prompt = (
        f"Using the following data sample from '{filename}':\n\n{data_sample}\n\n"
        f"Analyze the data and suggest the most suitable machine learning model "
    ...
    # Enhanced regex to capture model name without relying on asterisks
    model_match = re.search(r'([A-Za-z]+(?:\s[A-Za-z]+)*)', analysis)
    if model_match:
        recommended_model = model_match.group(1).replace(" ", "")
    else:
        recommended_model = None  # Avoid defaulting to a hardcoded model
    
    # If no model is recommended, prompt the LLM again or handle accordingly
    if not recommended_model:
        logging.warning("No model was recommended by the LLM. Using default model selection logic.")
        # Optionally, you can implement additional logic to choose a model based on data characteristics
        recommended_model = "LogisticRegression"  # Fallback option
...
def fix_code(code, error_output, iteration, recommended_model="LogisticRegression"):
    ...
    df_columns = ['CustomerID', 'Genre', 'Age', 'Annual Income (k$)', 'Spending Score (1-100)']
    ...
    # Save the raw data for auditing/debugging
...
def identify_target_variable(df):
    ...
    """
    # Example logic: Assume the target variable is the column with the least unique values
