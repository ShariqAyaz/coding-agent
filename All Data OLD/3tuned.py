...
def generate_initial_code():
    ...
    reasoning_prompt = (
        ...
        f"Analyze the data and suggest the most suitable 1 machine learning model to use "
        f"for this dataset. Provide a brief reasoning for your choice."
    )
    reasoning_system_prompt = "Analyze the data and recommend the best ML model and choose one."
    ...
    code_prompt = (
        ...
        f"perform preprocessing, split the data into training and testing sets, and apply the model. "
        f"Include code for evaluating and plotting the results. Do not include any comments in the code. "
        f"actual data is not limit to sample data actual data at file is big. Use the following data sample(10) for reference:\n\n"
        f"{data_sample}"
    )
...
def fix_code(code, error_output, iteration):
    ...
    llm_prompt = (
        ...
        f"Error message:\n{error_output}\n\n"
        f"Please fix the code. Only provide the corrected code without any explanations or comments."
    )
